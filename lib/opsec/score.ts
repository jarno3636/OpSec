...
import { readOwner, readEip1967Implementation, readErc20Meta, readTotalSupply, readBalanceOf } from "./onchain";
...
export async function computeReport(address: Address, raw: Raw): Promise<OpSecReport> {
  ...
  findings.push(P(!proxyDetected || hasImpl, 6, proxyDetected ? (hasImpl ? "Upgradeable proxy detected" : "Proxy w/out impl") : "No proxy risk detected", "proxy"));
  ...
  if (mainPair) {
    ...
    if (isFinite(buy) && isFinite(sell)) {
      ...
      findings.push(P(ratioOk, 5, trades < MIN_TRADES ? "Order flow ~thin market" : (ratioOk ? "Balanced 24h buy/sell" : "Skewed 24h order flow"), "buy_sell_ratio"));
    }
    ...
    if (isFinite(vol24h) && vol24h > 0) metrics["volume24hUSD"] = vol24h;
    if (isFinite(fdv) && fdv > 0) metrics["fdvUSD"] = fdv;
    if (liqUSD > 0 && fdv > 0) {
    ...
    if (lpAddr) {
      ...
      const tsN = num(ts);
      if (tsN > 0) {
        const lockedRaw = num(deadBal) + num(zeroBal) + lockers.reduce((s, b) => s + num(b), 0);
        const lockedPct = pct(lockedRaw, tsN);
        metrics["lpLockedPct"] = lockedPct;
  ...
  if (gpRec) {
    ...
    if (isFinite(taxSwing)) findings.push(P(Math.abs(taxSwing) <= 10, 3, `Tax swing Δ ${taxSwing}%`, "tax_swing"));
...
function first<T>(x: any): T | undefined { return Array.isArray(x?.result) ? x.result[0] : Array.isArray(x) ? x[0] : x?.result ?? x; }
...
function pickMainPair(pairs: any[]) {
  ...
  return pairs.slice().sort((a: any, b: any) => {
...
function suspiciousAirdrop(holders: any[]): boolean {
  ...
  let equalRuns = 0; for (let i = 1; i < q.length; i++) if (Math.abs(q[i] - q[i - 1]) < 1e-9) equalRuns++;
  return equalRuns > 20;
}
function gpOK(g: any) {
  if (!g) return false;
