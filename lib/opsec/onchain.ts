...
/* ---------- NEW: ERC-20 totalSupply & balanceOf (typed) ---------- */
...
export async function readTotalSupply(token: Address): Promise<bigint | undefined> {
...
export async function readBalanceOf(token: Address, holder: Address): Promise<bigint | undefined> {
  return await safeRead<bigint>(() =>
    (baseClient as any).readContract({
      address: token,
...
export async function readTaxHints(address: Address) {
  const [taxFee, buyTax, sellTax, totalBuyTax, totalSellTax] = await Promise.all([
    safeRead(() =>
      (baseClient as any).readContract({ address, abi: TAXY as any, functionName: "taxFee" })
    ),
    safeRead(() =>
      (baseClient as any).readContract({ address, abi: TAXY as any, functionName: "buyTax" })
    ),
    safeRead(() =>
      (baseClient as any).readContract({ address, abi: TAXY as any, functionName: "sellTax" })
    ),
    safeRead(() =>
      (baseClient as any).readContract({ address, abi: TAXY as any, functionName: "totalBuyTax" })
    ),
    safeRead(() =>
      (baseClient as any).readContract({ address, abi: TAXY as any, functionName: "totalSellTax" })
    ),
  ]);
